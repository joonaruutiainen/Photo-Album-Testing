...
This module performs tests to web application's searchbar functionality on Google Chrome.
...
class TestInput:
    ...
    """
    def __init__(self, msg, start_date='', end_date='', expected_results=0):
        self.msg = msg
...
class Image:
    ...
    keywords : list[str]
        keywords that are saved to an image (default is an empty list)
        TODO: keyword search's functionality is not tested yet in any way
    """
...
def initialize_images(log):
    ...
    try:
        for i in range(30):
            if i < 9:
                date = '2018-06-0{:n}T12:00:00'.format(i+1)
                TEST_IMAGES.append(Image(str(i+1),date))
            else:
                date = '2018-06-{:n}T12:00:00'.format(i+1)
                TEST_IMAGES.append(Image(str(i+1),date))
        for i in range(19):
            if i < 9:
                date = '2018-07-0{:n}T12:00:00'.format(i+1)
                TEST_IMAGES.append(Image(str(i+1+30), date))
            else:
                date = '2018-07-{:n}T12:00:00'.format(i+1)
                TEST_IMAGES.append(Image(str(i+1+30), date))
...
def compare_start_date(id, date):
    ...
    if date == '':
        date += '2018-05-31'
    if date.find('T') == str(-1):
        return get_image_date(id) >= date + 'T00:00:00'
    else:
...
def compare_end_date(id, date):
    ...
    if date == '':
        date += '2018-07-20'
    if date.find('T') == str(-1):
        return get_image_date(id) <= date + 'T23:59:59'
    else:
        return get_image_date(id) <= date
...
def test_search_with_date(testObject, log, next_page=False):
    ...
    for image in loaded_images:
        ...
        if  (compare_start_date(id, testObject.start_date) 
            ...
            and testObject.expected_results != 0 ):
    ...
    try:       
        ...
        if first_result != loaded_images[0].web_element.get_attribute('src'):
            ...
            if testObject.results_found != testObject.expected_results:
                testObject.errors += 1
                log.write('Expected {} results, got {} \n'.format(testObject.expected_results, testObject.results_found))
